...
void DRV8874_init(const drv8874_config_t *config, drv8874_handle_t *ret_motor) {
    ...
    mcpwm_new_operator(&oper_config, &motor->oper);
    ...
    mcpwm_new_comparator(motor->oper, &cmpr_config, &motor->cmpr_in1);
    mcpwm_new_comparator(motor->oper, &cmpr_config, &motor->cmpr_in2);

    // 4. Create Generators for IN1 and IN2
    mcpwm_generator_config_t gen1_config = { .gen_gpio_num = config->pin_in1 };
    mcpwm_new_generator(motor->oper, &gen1_config, &motor->gen_in1);
    
    mcpwm_generator_config_t gen2_config = { .gen_gpio_num = config->pin_in2 };
    mcpwm_new_generator(motor->oper, &gen2_config, &motor->gen_in2);

    // 5. Configure Standard PWM Actions (High on Zero, Low on Compare)
    // This allows us to control the duty cycle purely by changing the compare value.
    mcpwm_generator_set_action_on_timer_event(motor->gen_in1, 
        MCPWM_GEN_TIMER_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, MCPWM_TIMER_EVENT_EMPTY, MCPWM_GEN_ACTION_HIGH));
    mcpwm_generator_set_action_on_compare_event(motor->gen_in1, 
        MCPWM_GEN_COMPARE_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, motor->cmpr_in1, MCPWM_GEN_ACTION_LOW));

    mcpwm_generator_set_action_on_timer_event(motor->gen_in2, 
        MCPWM_GEN_TIMER_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, MCPWM_TIMER_EVENT_EMPTY, MCPWM_GEN_ACTION_HIGH));
    mcpwm_generator_set_action_on_compare_event(motor->gen_in2, 
        MCPWM_GEN_COMPARE_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, motor->cmpr_in2, MCPWM_GEN_ACTION_LOW));

    // 6. Enable and Start the Timer
    mcpwm_timer_enable(motor->timer);
    mcpwm_timer_start_stop(motor->timer, MCPWM_TIMER_START_NO_STOP);
