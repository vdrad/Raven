/**
 * @file ble_manager.c
 * @brief Implementation of the BLE Manager using the NimBLE stack.
 */
...
static int ble_spp_server_gap_event(struct ble_gap_event *event, void *arg) {
    switch (event->type) {
        case BLE_GAP_EVENT_CONNECT:
            if (event->connect.status != 0) ble_spp_server_advertise();
...
static void ble_spp_server_host_task(void *param) {
    RAVEN_LOGI(TAG, "BLE Host Task Started");
    nimble_port_run();
...
void ble_manager_send_message(uint8_t *data, uint16_t size) {
    ...
    for (int i = 0; i <= CONFIG_BT_NIMBLE_MAX_CONNECTIONS; i++) {
        if (conn_handle_subs[i]) {
            ...
            if (rc != 0) RAVEN_LOGE(TAG, "Error sending notification: %d", rc);
        }
    }
}
